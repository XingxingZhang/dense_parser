...
local function main()
  local opts = getOpts()
  print(opts)
  local vocab = DepTreeUtils.createVocab(opts.train, opts.freq, opts.ignoreCase, opts.keepFreq)
  ...
  if opts.bidirectional then
    DepTreeUtils.deptree2hdf5Bidirectional(opts.train, h5out, 'train', vocab, opts.maxLen)
  else
    DepTreeUtils.deptree2hdf5(opts.train, h5out, 'train', vocab, opts.maxLen)
  end
  print('create training set done!')
  if opts.bidirectional then
    DepTreeUtils.deptree2hdf5Bidirectional(opts.valid, h5out, 'valid', vocab, opts.maxLen)
  else
    DepTreeUtils.deptree2hdf5(opts.valid, h5out, 'valid', vocab, opts.maxLen)
  end
  print('create validating set done!')
  if opts.bidirectional then
    DepTreeUtils.deptree2hdf5Bidirectional(opts.test, h5out, 'test', vocab, opts.maxLen)
  else
    DepTreeUtils.deptree2hdf5(opts.test, h5out, 'test', vocab, opts.maxLen)
  end
