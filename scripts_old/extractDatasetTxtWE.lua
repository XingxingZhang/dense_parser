...
local function extractTxtWE(srcvocabF, dstvocabF, WEPath, WEOutPath)
  local svocab = torch.load(srcvocabF)
  local dvocab = torch.load(dstvocabF)
  print(table.keys(svocab))
  print(table.keys(dvocab))
  ...
  while true do
    ...
    local word = fields[1]
    if svocab.word2idx[word] ~= nil or dvocab.word2idx[word] ~= nil then
      msize = msize + 1
      idx2word[msize] = word
      local v = {}
      for i = 2, width + 1 do
...
local function main()
  ...
  cmd:option('--srcvocab', '/afs/inf.ed.ac.uk/group/project/img2txt/encdec/dataset/PWKP/an_ner/PWKP_108016.tag.80.aner.train.src.vocab.tmp.t7', 'path for source dictionary')
  ...
  extractTxtWE(opts.srcvocab, opts.dstvocab, opts.wepath, opts.weoutpath)
end
