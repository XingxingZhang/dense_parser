local Replicate, parent = torch.class('PReplicate','nn.Module')

function Replicate:__init(dim, ndim)
   parent.__init(self)
   -- self.nfeatures = nf
   self.dim = dim or 1
   self.ndim = ndim
...
function Replicate:updateOutput(input_)
   local input, nf = unpack( input_ )
   self.nfeatures = nf
...
function Replicate:updateGradInput(input_, gradOutput)
   local input, nf = unpack(input_)
   self.nfeatures = nf
