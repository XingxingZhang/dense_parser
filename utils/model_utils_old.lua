...
function model_utils.add_table(to, from)
  assert(#to == #from)
  for i = 1, #to do
    to[i]:add(from[i])
  end
...
function model_utils.combine_encdec_fflstm_parameters( enc_fflstm, dec_fflstm )
    ...
    local function getFFLSTMParameters(fflstm, parameters, gradParameters)
      
      for _, node in ipairs(fflstm.forwardnodes) do
        -- check IF this is a module and the module is not a lookup table
        if node.data.module then
          -- 'enc_ngram_lookup' or node.data.annotations.name == 'dec_ngram_lookup'
          if node.data.annotations.name ~= 'enc_ngram_lookup' and node.data.annotations.name ~= 'dec_ngram_lookup' then
            local mp,mgp = node.data.module:parameters()
            if mp and mgp then
              for i = 1,#mp do
                table.insert(parameters, mp[i])
            ...
            print('[combine_encdec_fflstm_parameters] found ngram_lookup! ' .. node.data.annotations.name)
          end
        end
      end
    ...
    getFFLSTMParameters(enc_fflstm, parameters, gradParameters)
    getFFLSTMParameters(dec_fflstm, parameters, gradParameters)
...
-- share parameters of two lookupTables: lstm_lookup and ngram_lookup
